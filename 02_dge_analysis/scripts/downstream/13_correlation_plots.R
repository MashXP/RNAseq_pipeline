# [[02_dge_analysis/scripts/downstream/13_correlation_plots.R]]
...
if (length(args) == 0) {
  stop("Usage: Rscript 13_correlation_plots.R <Group> [Species]")
}
...
if (!is.null(p_romi) && !is.null(p_krom)) {
  ...
  ggsave(file.path(res_dir, "figures/13_lfc_correlation.png"), p_combined, 
         width = 14, height = 9, dpi = 300, bg = "white")
  
  message("[OK] Correlation plot saved -> 13_lfc_correlation.png")
}
