# [[02_dge_analysis/scripts/downstream/06_venn.R]]
...
if (length(args) == 0) {
  stop("Usage: Rscript 06_venn.R <Group> [Species]")
}
...
if (length(shared_targets_list) > 0) {
  ...
  write.csv(final_shared_df, 
            file = file.path(res_dir, "tables/06_shared_targets_stats.csv"),
            row.names = FALSE)
  message("[OK] Master shared targets table saved -> 06_shared_targets_stats.csv")
}

# Save the rigor summary table
write.csv(rigor_summary, 
          file = paste0(res_dir, "/tables/06_venn_rigor_stats.csv"),
          row.names = FALSE)
message("[OK] Rigor summary table saved -> 06_venn_rigor_stats.csv")

# 4. Save Individual and Combined Plots
if (length(venn_plots) > 0) {
  # Individual save
  for (name in names(venn_plots)) {
    ggsave(paste0(res_dir, "/figures/06_venn_", tolower(name), ".png"), venn_plots[[name]],
  ...
  ggsave(paste0(res_dir, "/figures/06_venn_combined.png"), p_combined,
         width = 10, height = 8 * length(venn_plots), dpi = 300, bg = "white")
  
  message("[OK] Combined Venn diagram saved -> 06_venn_combined.png")
}
