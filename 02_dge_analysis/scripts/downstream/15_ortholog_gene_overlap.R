# [[02_dge_analysis/scripts/downstream/15_ortholog_gene_overlap.R]]
...
human_results_dir  <- file.path(results_root, "02_dge_analysis", "human", "tables")
canine_results_dir <- file.path(results_root, "02_dge_analysis", "canine", "tables")
output_dir         <- file.path(results_root, "02_dge_analysis", "comparative")
...
human_gtf  <- file.path(data_dir, "genome", "Human", "Homo_sapiens.GRCh38.113.gtf")
canine_gtf <- file.path(data_dir, "genome", "Canine", "Canis_lupus_familiaris.ROS_Cfam_1.0.113.gtf")
...
for (i in 1:nrow(comparisons)) {
  ...
  ggsave(file.path(plots_dir, paste0("15_ortholog_scatter_", safe_name, ".png")), p, width = 8, height = 7, bg = "white")
  
  # B. Export Tables
  write_csv(merged, file.path(tables_dir, paste0("15_ortholog_merged_", safe_name, ".csv")))
}
...
if (nrow(master_df) >= 2) {
  ...
  png(file.path(plots_dir, "15_conserved_ortholog_heatmap.png"), width = 1200, height = 1600, res = 150)
  pheatmap(
    heatmap_mat,
    cluster_rows = TRUE,
    cluster_cols = FALSE,
...
write_csv(summary_stats, file.path(tables_dir, "15_ortholog_overlap_summary.csv"))
