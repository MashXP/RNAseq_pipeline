# [[02_dge_analysis/scripts/downstream/11_ora_dotplot.R]]
...
if (length(args) == 0) {
  stop("Usage: Rscript 11_ora_dotplot.R <Group> [Species]")
}
...
if (!is.null(ora_combined) && nrow(ora_combined) > 0) {
  ...
  for (contrast in names(enrichment_results_all)) {
    ...
    if (nrow(df_dose) > 0) {
      ...
      ggsave(
        paste0(res_dir, "/figures/11_ora_dotplot_", safe_name, ".png"),
  ...
  ggsave(
    paste0(res_dir, "/figures/11_ora_dotplot_combined.png"),
    combined_p,
    width  = 5 * n_contrasts + 3,
    height = max(8, n_paths * 0.45),
    limitsize = FALSE
  )
  message("[OK] Combined ORA dotplot saved -> 11_ora_dotplot_combined.png")
