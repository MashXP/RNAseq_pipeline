# [[02_dge_analysis/scripts/downstream/12_upset_consistency.R]]
...
if (length(args) == 0) {
  stop("Usage: Rscript 12_upset_consistency.R <Group> [Species]")
}
...
if (length(all_sets) >= 2) {
  ...
  png(file.path(res_dir, "figures/12_upset_consistency.png"), 
  ...
  dev.off()
  message("[OK] UpSet plot saved -> 12_upset_consistency.png")
} else {
