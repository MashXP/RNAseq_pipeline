# [[03_as_analysis/scripts/downstream/01_rmats_summary.R]]
...
rdata_dir <- file.path(data_dir, "RData", "03_as_analysis")
# -----------------------------

# --- Pathing ---
RMATS_RESULTS_DIR <- file.path(data_dir, "rMATS-turbo/results")
GLOBAL_RESULTS_DIR <- file.path(results_root, "03_as_analysis/alt_splicing")
...
if (!dir.exists(RMATS_RESULTS_DIR)) {
  stop("rMATS results directory not found: ", RMATS_RESULTS_DIR)
}
...
if (length(summary_data) == 0) {
  message("Warning: No rMATS results found. Skipping plot generation.")
} else {
  ...
  # Calculate total counts per AS type
  ...
  # 2. Add Legend for Shortened names
