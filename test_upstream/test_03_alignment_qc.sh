# [[test_upstream/test_03_alignment_qc.sh]]
...
LOG_DIR="$BASE_DIR/../_data/logs/qc_test"
...
run_qc() {
    ...
    if [ "$sample_species" == "Human" ]; then
        # --- DEVIATION: Select test subset RefFlat instead of primary assembly
        REFFLAT="$GENOME_DIR/Human/chr21.refFlat"
    else
        # --- DEVIATION: Select test subset RefFlat instead of primary assembly
        REFFLAT="$GENOME_DIR/Dog/chr38.refFlat"
    fi
