...
# Goal: Generate PCA, Volcano, and Heatmap plots for MiaPAca-2 test data.
...
p_pca <- plotPCA(vsd, intgroup = "condition") +
  ...
  ggtitle("[TEST] PCA Plot: MiaPAca-2 Drug Response (Chr21)")

ggsave("../results_test/figures/04_test_pca_plot.png", p_pca,
       width = 8, height = 6, bg = "white")
message("[TEST][OK] PCA plot saved -> 04_test_pca_plot.png  (8x6 in, ", length(cond_levels), " conditions)")
...
for (dose in names(results_list)) {
  ...
  pCut <- 0.5; fcCut <- 0.5  # RELAXED FOR TEST
  keyvals <- case_when(
  ...
  out_vol <- paste0("../results_test/figures/04_test_volcano_", safe_name, ".png")
  ggsave(out_vol, p_volcano, width = 10, height = 8, bg = "white")
  message("[TEST][OK] Volcano saved  -> 04_test_volcano_", safe_name, ".png",
...
if (length(volcano_plots) > 1) {
  combined_volcano <- wrap_plots(volcano_plots, nrow = 1) +
    plot_annotation(
      title = "[TEST] Volcano Plots: All Concentrations vs DMSO",
      theme = theme(plot.title = element_text(face = "bold", hjust = 0.5))
    )
  ggsave("../results_test/figures/04_test_volcano_combined.png",
         combined_volcano, width = 10 * length(volcano_plots), height = 9, bg = "white")
  message("[TEST][OK] Combined volcano -> 04_test_volcano_combined.png",
          "  (", length(volcano_plots), " panels, ", 10 * length(volcano_plots), "x9 in)")
}

# 4. Venn Diagram (3.1a)
# Extract significant gene sets for each dose
# Use relaxed thresholds to ensure overlap is visible in test
sig_list <- lapply(results_list, function(x) {
  rownames(subset(as.data.frame(x$shrunk), padj < 0.5 & abs(log2FoldChange) > 0.5))
})
...
if (length(sig_list) >= 2) {
  p_venn <- ggVennDiagram(sig_list) +
    ...
    ggtitle("[TEST] DEG Overlap: Kromastat Concentrations vs DMSO")
  
  ggsave("../results_test/figures/04_test_venn_diagram_doses.png", p_venn,
         width = 8, height = 8, bg = "white")
  message("[TEST][OK] Venn diagram  -> 04_test_venn_diagram_doses.png",
...
if (!is.null(top_go)) {
  ...
  for (i in seq_len(nrow(top_pathways))) {
    ...
    message("[TEST] Pathway '", path_name, "': ", length(genes_path), " genes found in VSD.")
  ...
  if (length(expanded_matrix_list) > 0) {
    ...
    # Auto-calculate canvas height from fixed components + 6mm per row
    px_per_mm    <- 150 / 25.4
    mm_legend    <- 0
    mm_title     <- 22
    mm_col_title <- 12
    ...
    png("../results_test/figures/04_test_heatmap_pathway_genes.png",
    ...
    draw(ht_pathway,
         ...
         column_title           = "[TEST] MiaPAca-2: Genes Grouped by Pathway",
    ...
    dev.off()
    message("[TEST][OK] Pathway heatmap -> 04_test_heatmap_pathway_genes.png",
...
# 6. Default Heatmap (Top 30 Variable) with ComplexHeatmap
top_genes_idx <- head(order(rowVars(assay(vsd)), decreasing = TRUE), 30)
...
png("../results_test/figures/04_test_heatmap_top_variable.png",
...
draw(ht_var,
     ...
     merge_legend           = FALSE,
     align_heatmap_legend   = "heatmap_top",
     column_title           = "[TEST] Top 30 Most Variable Genes",
     column_title_gp        = gpar(fontsize = 12, fontface = "bold"))
dev.off()
message("[TEST][OK] Top-var heatmap  -> 04_test_heatmap_top_variable.png",
...
message("[TEST] Building Hallmark NES plots...")
...
for (dose in names(enrichment_results_all)) {
  ...
  if (!is.null(hallmark_res) && nrow(as.data.frame(hallmark_res)) > 0) {
    ...
    ggsave(
      paste0("../results_test/figures/04_test_hallmark_nes_", safe_name, ".png"),
...
if (length(nes_plots) > 1) {
  combined_nes <- wrap_plots(nes_plots, nrow = 1) +
    plot_annotation(title = "[TEST] Hallmark NES: All Concentrations",
                    theme = theme(plot.title = element_text(face = "bold", hjust = 0.5)))
  ggsave("../results_test/figures/04_test_hallmark_nes_combined.png",
         combined_nes, width = 10 * length(nes_plots), height = 12)
}

# 8. GSEA Dotplots -- individual per dose + stitched combined
message("[TEST] Building GSEA dotplots...")
...
if (!is.null(gsea_combined) && nrow(gsea_combined) > 0) {
  ...
  for (dose in levels(factor(gsea_combined$Dose, levels = c("0.5nM (500pM)", "1nM", "5nM")))) {
    ...
    if (nrow(df_dose) > 0) {
      ...
      # Save individual plot
      ggsave(
        paste0("../results_test/figures/04_test_gsea_dotplot_", safe_name, ".png"),
  ...
  if (length(gsea_plots) > 1) {
    combined_gsea <- wrap_plots(gsea_plots, nrow = 1, guides = "collect") +
      plot_annotation(
        title = "[TEST] GSEA: Multi-Dose Comparison",
        theme = theme(plot.title = element_text(face = "bold", hjust = 0.5))
      )
    ggsave(
      "../results_test/figures/04_test_gsea_dotplot_combined.png",
      combined_gsea,
...
message("[TEST] Visualization complete. All plots saved to ../results_test/figures/")
