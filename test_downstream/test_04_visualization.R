# [[test_downstream/test_04_visualization.R]]
# Goal: Generate PCA, Volcano, and Heatmap plots for specific group.
...
if (length(args) < 2) {
  stop("Usage: Rscript 04_visualization.R <Group> <Species>")
}
group_name <- args[1]
species_name <- args[2]
...
# 1. Load data
# --- DEVIATION: Load from test-specific directories
...
# Combine Treatment and Sample info if needed
...
# --- DEVIATION: Replaced hardcoded '5nM' with dynamic fallback to last test contrast
test_dose <- tail(names(enrichment_results_all), 1)
top_go <- enrichment_results_all[[test_dose]]$ora_go
if (!is.null(top_go)) {
  # --- DEVIATION: Relaxed pathway extraction threshold from p.adjust < 0.05 to pvalue < 0.2
...
if (!is.null(gsea_combined) && nrow(gsea_combined) > 0) {
  ...
  gsea_plots <- list()
  for (dose in levels(factor(gsea_combined$Dose, levels = c("Romi (6 nM)", "GNL", "NanoRomi (6 nM)")))) {
...
message("Visualization complete. All plots saved to ", res_dir, "/figures/")
