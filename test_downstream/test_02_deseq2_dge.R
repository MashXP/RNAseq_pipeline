# [[test_downstream/test_02_deseq2_dge.R]]
...
if (length(args) < 2) {
  stop("Usage: Rscript test_02_deseq2_dge.R <Group> <Species>")
}
group_name <- args[1]
species_name <- args[2]
...
load(paste0("./.RData_test/", group_name, "/01_processed_counts.RData"))
...
# --- DEVIATION: Output to test-specific directories
save(dds, results_list, file = paste0("./.RData_test/", group_name, "/02_deseq_results.RData"))
