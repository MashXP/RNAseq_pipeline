...
The PCA plot visualizes the overall transcriptomic variance across the canine samples.

![Combined PCA Plot|600](../../results/canine/figures/04_01_pca_combined.png)

### 1.1 Interpretation:
- **PC1 (96% Variance)**: The dominant source of variance is **Cell Line Identity**. CNK89 (NK-cell cancer) and UL1 (T-ALL cancer) lineages are sharply bifurcated along the primary axis, indicating significant baseline transcriptomic differences between these canine lymphoid models.
- **PC2 (3% Variance)**: This axis captures the **Drug Treatment Effect**.
    - Both canine cell lines respond to treatment with a consistent vertical shift along PC2.
...
| :---: | :---: |
| ![CNK89 PCA](../../results/canine/figures/04_01_pca_CNK89.png) | ![UL1 PCA](../../results/canine/figures/04_01_pca_UL1.png) |

## 2. Differential Gene Expression (Volcano Plots)

### 2.1 Global Response Dashboard
The combined dashboard provides a bird's-eye view of all primary contrasts used to validate the drug response in canine lymphoid models.
...
> [!NOTE]
> Global DEG counts and ratios are derived from the species-level analysis consolidated in: [04_03_venn_rigor_stats.csv](../../results/canine/tables/04_03_venn_rigor_stats.csv).
...
Both inhibitors show a significant bias toward gene activation in canine cells. However, Kromastat's response is exceptionally skewed, with almost no gene suppression observed at this high-stringency concentration.
...
- **Potency Proof**: With **10 significant differences** between the two drugs at the same molarity, the canine data confirms that these inhibitors are not functionally redundant.
...
> - **DGE Summary**: [04_02_dge_summary_stats.csv](../../results/canine/tables/04_02_dge_summary_stats.csv)
> - **Top Genes**: [04_02_top_dge_genes.csv](../../results/canine/tables/04_02_top_dge_genes.csv)
...
2. **Conserved Targets**: Highly significant targets such as `ENPP2` and `HOOK2` are consistently modulated across both canine cell lines.
...
> Full stats: [04_03_shared_targets_stats.csv](../../results/canine/tables/04_03_shared_targets_stats.csv)
...
| :------------------------------------------------------------- | :----------------------------------------------------------- | :--------------------------------------------------------------- |
| ![Venn All\|400](../../results/canine/figures/04_03_venn_all.png) | ![Venn Up\|400](../../results/canine/figures/04_03_venn_up.png) | ![Venn Down\|400](../../results/canine/figures/04_03_venn_down.png) |
...
**Finding**: Kromastat acts primarily as a **transcriptomic activation subset** of Romidepsin in canine cell lines. Approximately **94% of Kromastat's total activation response is contained within the Romidepsin signature**.
...
![GSEA Hallmark Combined|1132](../../results/canine/figures/04_07_gsea_dotplot_combined.png)
...
| :---: | :---: |
| ![UL1 Kromastat Dotplot\|500](../../results/canine/figures/04_07_gsea_dotplot_UL1_Kromastat_6nM_vs_DMSO_Kromastat.png) | ![CNK89 Kromastat Dotplot\|500](../../results/canine/figures/04_07_gsea_dotplot_CNK89_Kromastat_6nM_vs_DMSO_Kromastat.png) |
...
![UpSet Consistency Matrix|717](../../results/canine/figures/04_09_upset_consistency.png)
...
![LFC Correlation|986](../../results/canine/figures/04_10_lfc_correlation.png)
...
Individual ORA dotplot files are preserved in `results/canine/figures/04_08_ora_dotplot_*.png` for reference.
...
1. **The Shared Core (The Proliferation Crash)**: Both drugs successfully execute the primary function of HDAC inhibition — a massive transcriptomic inversion shutting down proliferative pathways (MYC Targets, E2F Targets, G2M Checkpoint) across all cellular backgrounds *(see Sections [[downstream_analysis_canine#4.1 Global Functional Response (Signature Matrices)|4.1]], [[downstream_analysis_canine#6.1 Global Functional Response|6.1]], [[downstream_analysis_canine#7.1 Global Overview|7.1]])*.
2. **The Potency Gap**: Romidepsin acts as a "Metabolic Crusher," driving a deeper, more aggressive transcriptional shutdown — particularly in UL1 (**5,045 DEGs**) — while CNK89 shows reduced sensitivity (**2,245 DEGs**). Kromastat acts as a "Partial Modulator," achieving significant but milder suppression, with CNK89's response (**589 DEGs**) representing a striking case of transcriptomic resistance *(see Sections [[downstream_analysis_canine#1.1 Interpretation:|1.1]], [[downstream_analysis_canine#2.1.2 Global Magnitude of Response|2.1.2]], [[downstream_analysis_canine#5.2 Drug Effect: The "Potency Gap" at Single-Gene Resolution|5.2]])*.
3. **Divergent Immune Modulation**: Romidepsin broadly represses immune-related signalling (Allograft Rejection, Interferon Alpha/Gamma) across both lineages. Conversely, Kromastat actively amplifies immune-receptor pathways — including IL2-STAT5, IL6-JAK-STAT3, and Inflammatory Response — particularly in the CNK89 NK-cell cancer background, a phenotype entirely absent from Romidepsin *(see Sections [[downstream_analysis_canine#4.4 Direct Comparison: Romidepsin vs. Kromastat|4.4]], [[downstream_analysis_canine#6.4 Direct Comparison: Romidepsin vs. Kromastat|6.4]], [[downstream_analysis_canine#7.2 Local Response: Cross-Contrast Comparison|7.2]])*.
4. **The Subset Relationship**: Approximately **86% of Kromastat's total transcriptomic response is contained within the Romidepsin signature**, confirming that Kromastat operates as a transcriptomic subset of Romidepsin in canine cells — rarely engaging pathways not already captured by Romidepsin *(see Section [[downstream_analysis_canine#3.3 The "Subset" Hypothesis|3.3]])*.
...
- **Romidepsin** is the superior **direct cytotoxic agent**. Its aggressive repression of both the cancer signature *(see Section [[downstream_analysis_canine#5.2 Drug Effect: The "Potency Gap" at Single-Gene Resolution|5.2]])* and immune signalling *(see Sections [[downstream_analysis_canine#4.4 Direct Comparison: Romidepsin vs. Kromastat|4.4]], [[downstream_analysis_canine#6.4 Direct Comparison: Romidepsin vs. Kromastat|6.4]])* makes it highly effective at dismantling tumor metabolism — but its immunosuppressive nature may limit its long-term efficacy by dampening the anti-tumor immune response.
- **Kromastat** is a potential **immune-modulating agent**, but faces a significant **Efficacy Paradox**. While it executes a partial anti-proliferative program *(see Sections [[downstream_analysis_canine#1.1 Interpretation:|1.1]], [[downstream_analysis_canine#4.1 Global Functional Response (Signature Matrices)|4.1]])* without broad immunosuppression, its robust "immune activator" signature (IFN, IL2-STAT5) is currently lineage-dependent (specifically in the T-cell lineage) *(see Sections [[downstream_analysis_canine#4.5 Summary of Functional Pathway Findings|4.5]], [[downstream_analysis_canine#6.5 Summary of Hallmark Findings|6.5]])*. In the malignant NK-cell CNK89 background, Kromastat's response is severely compressed (~589 DEGs), indicating resistance at 6nM *(see Section [[downstream_analysis_canine#2.3 Local (Cell-Line Specific) Response|2.3]])*. Its utility as a cancer therapeutic likely depends on dose-escalation or specific combination strategies to overcome this lineage-specific resistance while leveraging its immune-amplifying properties in compatible cellular backgrounds.
