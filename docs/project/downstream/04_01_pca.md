...
## 0. Data Flow (I/O)
- **Input**: 
    - **Results RData**: `scripts_downstream/.RData/[Group]/02_deseq_results.RData` (Contains the `dds` object).
- **Processing**: Variance Stabilizing Transformation (VST), Principal Component calculation.
- **Output**: 
    - **QC Figures**: `../results/[Group]/figures/04_pca_combined.png` and `04_pca_[CellLine].png`.
...
- **Output**: 
    - `04_pca_combined.png`: The big-picture overview.
    - `04_pca_[CellLine].png`: The high-resolution focus on drug efficacy within specific models.
