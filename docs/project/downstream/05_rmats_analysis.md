...
## 2. Candidate Ranking
To narrow down thousands of events to the most biologically relevant and visually robust candidates, `utils/rank_as_candidates.R` is used. For a detailed breakdown of the ranking algorithm, see [Downstream Utility Dissection](file:///mnt/22T/phongdinh/RNAseq_pipeline/docs/project/downstream/utils_dissection.md#1-rank_as_candidatesr).
...
### Usage:
```bash
# bash scripts_downstream/05_02_rmats_sashimi.sh <cell_line> <comparison> <as_type> <event_id>
bash scripts_downstream/05_02_rmats_sashimi.sh CNK89 Kromastat_6nM_vs_Romidepsin_6nM SE 2318
```
